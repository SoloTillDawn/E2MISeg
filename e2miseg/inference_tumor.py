...
def test(fold):
    ...
    label_files = glob.glob(os.path.join(label_path, 'BRATS_*.nii.gz'))
    infer_files = glob.glob(os.path.join(infer_path, 'BRATS_*.nii.gz'))
    ...
    savepath=" "
    file=savepath + 'inferTs_out_7777_loss1.45v1_hfr31best/'

    if not os.path.exists(file):
        os.makedirs(file)
    fw = open(file+'/dice_tumor.txt', 'w')
