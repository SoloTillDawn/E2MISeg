...
from e2miseg.experiment_planning.common_utils import split_4d_nifti
from e2miseg.paths import nnFormer_raw_data, nnFormer_cropped_data, preprocessing_output_dir
from e2miseg.preprocessing.cropping import ImageCropper
...
def split_4d(input_folder, num_processes=default_num_threads, overwrite_task_output_id=None):
    assert isdir(join(input_folder, "imagesTr")) and isdir(join(input_folder, "labelsTr")) and \
           isfile(join(input_folder, "dataset.json")), "dir error"
...
def plan_and_preprocess(task_string, processes_lowres=default_num_threads, processes_fullres=3, no_preprocessing=False):
    from e2miseg.experiment_planning.experiment_planner_baseline_2DUNet import ExperimentPlanner2D
