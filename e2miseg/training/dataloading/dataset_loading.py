...
def load_dataset(folder, num_cases_properties_loading_threshold=1000):
    ...
    print('dataset_loading.py : loading dataset')
    case_identifiers = get_case_identifiers(folder)
    ...
    if len(case_identifiers) <= num_cases_properties_loading_threshold:
        print('loading all case properties(属性)')
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        # (which is what the network will get) these patches will also cover the border(边界) of the patients
        self.need_to_pad = (np.array(patch_size) - np.array(final_patch_size)).astype(int)
        if pad_sides is not None:
            if not isinstance(pad_sides, np.ndarray):
                pad_sides = np.array(pad_sides)
            self.need_to_pad += pad_sides
        self.memmap_mode = memmap_mode  # r： only read
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            # (Lung for example)
            if self.get_do_oversample(j):  # not batch_idx < round(self.batch_size * (1 - self.oversample_foreground_percent))
                force_fg = True
            else:
                force_fg = False
