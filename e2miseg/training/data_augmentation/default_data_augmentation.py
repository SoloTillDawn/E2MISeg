...
default_3D_augmentation_params = {
    ...
    "num_threads": 20 if 'nnFormer_n_proc_DA' not in os.environ else int(os.environ['nnFormer_n_proc_DA']),
...
def get_patch_size(final_patch_size, rot_x, rot_y, rot_z, scale_range):
    ...
    final_shape = np.copy(coords) # [ 64 128 128]
    if len(coords) == 3:
...
def get_default_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
    ...
    if regions is not None:
        tr_transforms.append(ConvertSegmentationToRegionsTransform(regions, 'target', 'target'))# 将分割转换为区域（根据给定的区域定义）。
...
if __name__ == "__main__":
    from e2miseg.training.dataloading.dataset_loading import DataLoader3D, load_dataset
