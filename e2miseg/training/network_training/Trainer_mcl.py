...
from e2miseg.inference.segmentation_export import save_segmentation_nifti_from_softmax
...
from e2miseg.postprocessing.connected_components import determine_postprocessing
from e2miseg.training.data_augmentation.default_data_augmentation import default_3D_augmentation_params, \
    default_2D_augmentation_params, get_default_augmentation, get_patch_size
from e2miseg.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset
from e2miseg.training.loss_functions.dice_loss import DC_and_CE_loss, SSDC_and_CE_loss
from e2miseg.training.network_training.network_trainer_mcl import NetworkTrainer_mcl
from e2miseg.utilities.nd_softmax import softmax_helper
from e2miseg.utilities.tensor_utilities import sum_tensor
...
class Trainer_mcl(NetworkTrainer_mcl):
...
进行推理，则为None
        ...
        IMPORTANT: 如果你继承了nnFormerTrainer，并且init参数改变了，那么你需要重新定义self.init_args 相应地在init中。
                  否则检查点将无法正常加载!
        """
        super(Trainer_mcl, self).__init__(deterministic, fp16)
        ...
        self.loss = SSDC_and_CE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {},
                                      weight_dice=2.0, class2=False)
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        print("Trainer2_MODEL:", self.network.__class__.__name__)
        print("Trainer2_optimizer:", self.optimizer.__class__.__name__)
        print("Trainer2_lr_scheduler:", self.lr_scheduler.__class__.__name__)
    ...
    def run_training(self):
        ...
        super(Trainer_mcl, self).run_training()
    ...
    def process_plans(self, plans):
        ...
        self.transpose_forward = plans['transpose_forward'] # (xwp)
        self.transpose_backward = plans['transpose_backward'] # (xwp)
    ...
    def preprocess_patient(self, input_files):
        ...
        if preprocessor_name is None:
            if self.threeD:
                preprocessor_name = "GenericPreprocessor" #preprocessing
            else:
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(Trainer_mcl, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
