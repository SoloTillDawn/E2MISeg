...
class NetworkTrainer_mcl(object):
    def __init__(self, deterministic=True, fp16=False):
        """
        A generic class that can train almost any neural network (RNNs excluded). It provides basic functionality such
        as the training loop(循环训练), tracking of training and validation losses (and the target metric(目标度量) if you implement it)
        ...
        results.这是基于损失/度量的移动平均线(MA)，而不是原始值，以获得更平滑结果。
        ...
        self.optimizer = None # (xwp) 
        self.lr_scheduler = None # (xwp)
        ...
        self.train_loss_MA_alpha = 0.93  # (xwp) alpha * old + (1-alpha) * new
        self.train_loss_MA_eps = 5e-4  # (xwp) new MA must be at least this much better (smaller)
        ...
        self.train_loss_MA = None # (xwp)
        self.best_val_eval_criterion_MA = None # (xwp)
        self.best_MA_tr_loss_for_patience = None # (xwp) 
        self.best_epoch_based_on_MA_tr_loss = None # (xwp)
        self.all_tr_losses = [] # (xwp)
        self.all_val_losses = [] # (xwp)
        self.all_val_losses_tr_mode = [] # (xwp)
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        torch.save(save_this, fname)  # 保存 checkpoint

        self.print_to_log_file("done, saving took %.2f seconds" % (time() - start_time))
        if(self.epoch % 50 == 0):
            self.print_to_log_file("done, saving took %.2f seconds.预估还需要: %.2f h" % (time() - start_time, ((time() - start_time)*(1000-self.epoch))/3600))
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        :return:
        """
        print('NetworkTrainer_synapse.load_checkpoint_ram : I am here !!!')
        if not self.was_initialized:
            self.initialize(train)
    ...
    def update_eval_criterion_MA(self):
        """
        用于更新用于早停（early stopping）的评估准则的移动平均值（MA）
