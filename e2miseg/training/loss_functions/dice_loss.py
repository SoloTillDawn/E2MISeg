import torch
from e2miseg.training.loss_functions.TopK_loss import TopKLoss
from e2miseg.training.loss_functions.crossentropy import RobustCrossEntropyLoss
from e2miseg.utilities.nd_softmax import softmax_helper, sigmod_helper
...
DC_and_CE_loss:
DC_and_BCE_loss(X):
GDL_and_CE_loss(RobustCrossEntropyLoss):
DC_and_topk_loss(SoftDiceLoss/SoftDiceLossSquared):
"""
class GDL(nn.Module):
...
class SSDCLoss(nn.Module):
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        alpha = (torch.min(pred_sum, target_sum)+dis+self.smooth)/(torch.max(pred_sum, target_sum)+dis+self.smooth)
        ...
        if not self.do_bg:
            if self.batch_dice:
                dc = dc[1:]
                siou_loss = alpha[1:] * dc
            else:
                dc = dc[:, 1:]
                siou_loss = alpha[:, 1:] * dc

        siou_loss = siou_loss.mean()
...
class SSDC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1.6,
        ...
        if not square_dice:
            if self.class2 == True:
                self.ssdc = SSDCLoss(apply_nonlin=sigmod_helper, **soft_dice_kwargs)
            else:
                self.ssdc = SSDCLoss(apply_nonlin=softmax_helper, **soft_dice_kwargs)
        else:
            self.ssdc = SoftDiceLossSquared(apply_nonlin=softmax_helper, **soft_dice_kwargs)
    ...
    def forward(self, net_output, target):
        if self.ignore_label is not None:
            ...
            mask = None
        ssdc_loss = self.ssdc(net_output, target, loss_mask=mask) if self.weight_dice != 0 else 0

        if self.log_dice:
            ssdc_loss = -torch.log(-ssdc_loss)
        ce_loss = self.ce(net_output, target[:, 0].long()) if self.weight_ce != 0 else 0
