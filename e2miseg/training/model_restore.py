...
import importlib
import pkgutil
from e2miseg.paths import network_training_output_dir, preprocessing_output_dir, default_plans_identifier
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None,folder=None):
    ...
    if len(init)>10:
        ...
        del init[2]
        del init[-2]
    search_in = join(e2miseg.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="e2miseg.training.network_training")
    ...
    if tr is None:
        ...
        try:
            ...
            tr = recursive_find_python_class([search_in], name, current_module="e2miseg.model_training")
        except ImportError:
            pass

    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in e2miseg.trainig.network_training.  "
                           "please move it or change the code of restore_model. "
                           "e2miseg.trainig.network_training."
