...
from e2miseg.network_architecture.acdc.model_components import MFGA, E2MISEncoder, E2MISDecoder
...
class E2MISeg(SegmentationNetwork):
    def __init__(
        ...
        self.e2mis_encoder = E2MISEncoder(dims=dims, depths=depths, num_heads=num_heads)
    ...
    def proj_feat(self, x, hidden_size, feat_size):
        x = x.view(x.size(0), feat_size[0], feat_size[1], feat_size[2], hidden_size)  # 2,2,5,5,256
        x = x.permute(0, 4, 1, 2, 3).contiguous()
        return x

    def forward(self, x_in):
        x_output, hidden_states = self.e2mis_encoder(x_in)
